                         ...
                         if (c != 0)
                         {
                              ...
                              for (var i = 0; i < found.length; i++)
                              {
                                   ...
                                   if (l_o >= start && l_o < set.show + start)
                                   {
                                        ...
                                        //Format the text preview by adding spaces between joined sentences.
                                        ...
                                        else
                                        {
                                             ...
                                             for (var j = 0; j < d_w.length; j++){
                                                  //Compare each search term d_w to each word in the text t_w.
                                                  //We strip out HTML tags in case each instance of the search term is bolded as 
                                                  //per the highlighting settings
                                                  for (var k = 0; k < t_w.length; k++){
                                                       if (k < startIndex && t_w[k].replace(/<(?:.|\n)*?>|[,\.!()\?:=]/gm, '').toUpperCase() === d_w[j].toUpperCase())
                                                            startIndex = k;
                                                  }
                                             }

                                             //If the descriptive text is longer than the number of words displayed, but we don't
                                             //have any search terms in the text (only in the title), we set the startindex to zero.
                                             if (startIndex === Number.MAX_VALUE)
                                                  startIndex = 0;
                                             
                                             //Now we work back from this start index until we find the start of a sentence.
                                             var startLetter;
                                             do { startLetter = t_w[startIndex--].replace(/<(?:.|\n)*?>/gm, '').charAt(0); } 
                                             while ((!(startLetter.toUpperCase() >= 'A' && startLetter.toUpperCase() <= 'Z') && startIndex >= 0) 
                                                  || (startLetter != startLetter.toUpperCase() && startIndex >= 0));
                                             startIndex++;

                                             for (var f = startIndex; f < startIndex + set.descriptiveWords 
                                                  && (startIndex + set.descriptiveWords) < t_w.length ; f++)
                                             {
                                                  t_d += t_w[f] + ' '; 	
                                             }
                                        }
                              ...
                              if (c > set.show)
                              {
                                   ...
                                   if (page + 1 != pages)
                                   {
                                       out += '<li><a href="javascript:void(0)" class="tipue_search_foot_box" id="' + (start + set.show) + '_' + replace + '">Next <i class="fa fa-angle-right"></a></li>'; 
                                   }                    
